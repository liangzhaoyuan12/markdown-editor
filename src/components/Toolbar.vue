<template>
  <div class="toolbar" ref="toolbarRef">
    <div class="toolbar-group">
      <button @click="handleAction('undo')" title="撤销 (Ctrl+Z)" class="toolbar-btn">
        <svg viewBox="0 0 24 24" width="18" height="18"><path fill="currentColor" d="M12.5 8C9.85 8 7.45 9 5.6 10.6L2 7V16h9l-3.62-3.62c1.39-1.16 3.16-1.88 5.12-1.88 3.54 0 6.55 2.31 7.6 5.5l2.37-.78C21.08 11.03 17.15 8 12.5 8z"/></svg>
      </button>
      <button @click="handleAction('redo')" title="重做 (Ctrl+Y)" class="toolbar-btn">
        <svg viewBox="0 0 24 24" width="18" height="18"><path fill="currentColor" d="M18.4 10.6C16.55 9 14.15 8 11.5 8c-4.65 0-8.58 3.03-9.96 7.22L3.9 16c1.05-3.19 4.05-5.5 7.6-5.5 1.95 0 3.73.72 5.12 1.88L13 16h9V7l-3.6 3.6z"/></svg>
      </button>
    </div>
    ...
    <div class="toolbar-group">
      <button @click="handleAction('bold')" title="粗体 (Ctrl+B)" class="toolbar-btn">
      ...
      <button @click="handleAction('strikethrough')" title="删除线" class="toolbar-btn">
        <svg viewBox="0 0 24 24" width="18" height="18"><path fill="currentColor" d="M10 19h4v-3h-4v3zM5 4v3h5v3h4V7h5V4H5zM3 14h18v-2H3v2z"/></svg>
      </button>
      <button @click="handleAction('italic')" title="斜体 (Ctrl+I)" class="toolbar-btn">
        <svg viewBox="0 0 24 24" width="18" height="18"><path fill="currentColor" d="M10 4v3h2.21l-3.42 8H6v3h8v-3h-2.21l3.42-8H18V4z"/></svg>
      </button>
      <button @click="handleAction('quote')" title="引用" class="toolbar-btn">
    ...
    <div class="toolbar-group">
      <button @click="handleAction('h1')" title="标题 1" class="toolbar-btn toolbar-btn-h">H1</button>
      <button @click="handleAction('h2')" title="标题 2" class="toolbar-btn toolbar-btn-h">H2</button>
      <button @click="handleAction('h3')" title="标题 3" class="toolbar-btn toolbar-btn-h">H3</button>
      <button @click="handleAction('h4')" title="标题 4" class="toolbar-btn toolbar-btn-h">H4</button>
      <button @click="handleAction('h5')" title="标题 5" class="toolbar-btn toolbar-btn-h">H5</button>
      <button @click="handleAction('h6')" title="标题 6" class="toolbar-btn toolbar-btn-h">H6</button>
    </div>

    <div class="toolbar-divider"></div>

    <div class="toolbar-group">
      <button @click="handleAction('ul')" title="无序列表" class="toolbar-btn">
      ...
      <button @click="handleAction('ol')" title="有序列表" class="toolbar-btn">
      ...
      <button @click="handleAction('hr')" title="横线" class="toolbar-btn">
    ...
    <div class="toolbar-group">
      <button @click="handleAction('link')" title="链接 (Ctrl+K)" class="toolbar-btn">
      ...
      <button @click="handleAction('image')" title="图片" class="toolbar-btn">
      ...
      <button @click="handleAction('code')" title="行内代码" class="toolbar-btn">
      ...
      <button @click="handleAction('codeblock')" title="代码块" class="toolbar-btn">
      ...
      <button @click="handleAction('table')" title="表格" class="toolbar-btn">
    ...
    <div class="toolbar-group toolbar-group-relative">
      <button @click="handleAction('datetime')" title="日期时间" class="toolbar-btn">
      ...
      <button @click="handleAction('emoji')" title="Emoji" class="toolbar-btn" :class="{ 'toolbar-btn-active': showEmojiPicker }">
      ...
      <div v-if="showEmojiPicker" class="emoji-picker" @click.stop>
        <div class="emoji-picker-header">
          ...
          <button @click="closeEmojiPicker" class="emoji-picker-close" title="关闭">
    ...
    <div class="toolbar-group">
      <button @click="handleAction('clear')" title="清空" class="toolbar-btn">
      ...
      <button @click="handleAction('search')" title="搜索 (Ctrl+F)" class="toolbar-btn">
...
const emojiCategories = {
  '表情': ['😀', '😃', '😄', '😁', '😆', '😅', '🤣', '😂', '🙂', '🙃', '😉', '😊', '😇', '🥰', '😍', '🤩', '😘', '😗', '😚', '😙', '😋', '😛', '😜', '🤪', '😝', '🤑', '🤗', '🤭', '🤫', '🤔', '🤐', '🤨', '😐', '😑', '😶', '😏', '😒', '🙄', '😬', '🤥', '😌', '😔', '😪', '🤤', '😴', '😷', '🤒', '🤕', '🤢', '🤮', '🤧', '🥵', '🥶', '🥴', '😵', '🤯', '🤠', '🥳', '😎', '🤓', '🧐', '😕', '😟', '🙁', '☹️', '😮', '😯', '😲', '😳', '🥺', '😦', '😧', '😨', '😰', '😥', '😢', '😭', '😱', '😖', '😣', '😞', '😓', '😩', '😫', '🥱', '😤', '😡', '😠', '🤬', '😈', '👿', '💀', '☠️', '💩', '🤡', '👹', '👺', '👻', '👽', '👾', '🤖'],
  '手势': ['👋', '🤚', '🖐️', '✋', '🖖', '👌', '🤌', '🤏', '✌️', '🤞', '🤟', '🤘', '🤙', '👈', '👉', '👆', '🖕', '👇', '☝️', '👍', '👎', '✊', '👊', '🤛', '🤜', '👏', '🙌', '👐', '🤲', '🤝', '🙏', '✍️', '💅', '🤳', '💪', '🦾', '🦿', '🦵', '🦶', '👂', '🦻', '👃', '🧠', '🫀', '🫁', '🦷', '🦴', '👀', '👁️', '👅', '👄', '👶', '🧒', '👦', '👧', '🧑', '👱', '👨', '🧔', '👨‍🦰', '👨‍🦱', '👨‍🦳', '👨‍🦲', '👩', '👩‍🦰', '🧑‍🦰', '👩‍🦱', '🧑‍🦱', '👩‍🦳', '🧑‍🦳', '👩‍🦲', '🧑‍🦲', '👱‍♀️', '👱‍♂️', '🧓', '👴', '👵', '🙍', '🙍‍♂️', '🙍‍♀️', '🙎', '🙎‍♂️', '🙎‍♀️', '🙅', '🙅‍♂️', '🙅‍♀️', '🙆', '🙆‍♂️', '🙆‍♀️', '💁', '💁‍♂️', '💁‍♀️', '🙋', '🙋‍♂️', '🙋‍♀️', '🧏', '🧏‍♂️', '🧏‍♀️', '🙇', '🙇‍♂️', '🙇‍♀️', '🤦', '🤦‍♂️', '🤦‍♀️', '🤷', '🤷‍♂️', '🤷‍♀️'],
  '动物': ['🐵', '🐒', '🦍', '🦧', '🐶', '🐕', '🦮', '🐕‍🦺', '🐩', '🐺', '🦊', '🦝', '🐱', '🐈', '🐈‍⬛', '🦁', '🐯', '🐅', '🐆', '🐴', '🐎', '🦄', '🦓', '🦌', '🦬', '🐮', '🐂', '🐃', '🐄', '🐷', '🐖', '🐗', '🐽', '🐏', '🐑', '🐐', '🐪', '🐫', '🦙', '🦒', '🐘', '🦣', '🦏', '🦛', '🐭', '🐁', '🐀', '🐹', '🐰', '🐇', '🐿️', '🦫', '🦔', '🦇', '🐻', '🐻‍❄️', '🐨', '🐼', '🦥', '🦦', '🦨', '🦘', '🦡', '🐾', '🦃', '🐔', '🐓', '🐣', '🐤', '🐥', '🐦', '🐧', '🕊️', '🦅', '🦆', '🦢', '🦉', '🦤', '🪶', '🦩', '🦚', '🦜', '🐸', '🐊', '🐢', '🦎', '🐍', '🐲', '🐉', '🦕', '🦖', '🐳', '🐋', '🐬', '🦭', '🐟', '🐠', '🐡', '🦈', '🐙', '🐚', '🐌', '🦋', '🐛', '🐜', '🐝', '🪲', '🐞', '🦗', '🪳', '🕷️', '🕸️', '🦂', '🦟', '🪰', '🪱', '🦠', '💐', '🌸', '💮', '🏵️', '🌹', '🥀', '🌺', '🌻', '🌼', '🌷'],
  '食物': ['🍏', '🍎', '🍐', '🍊', '🍋', '🍌', '🍉', '🍇', '🍓', '🫐', '🍈', '🍒', '🍑', '🥭', '🍍', '🥥', '🥝', '🍅', '🍆', '🥑', '🥦', '🥬', '🥒', '🌶️', '🫑', '🌽', '🥕', '🫒', '🧄', '🧅', '🥔', '🍠', '🥐', '🥯', '🍞', '🥖', '🥨', '🧀', '🥚', '🍳', '🧈', '🥞', '🧇', '🥓', '🥩', '🍗', '🍖', '🦴', '🌭', '🍔', '🍟', '🍕', '🥪', '🥙', '🧆', '🌮', '🌯', '🫔', '🥗', '🥘', '🫕', '🥫', '🍝', '🍜', '🍲', '🍛', '🍣', '🍱', '🥟', '🦪', '🍤', '🍙', '🍚', '🍘', '🍥', '🥠', '🥮', '🍢', '🍡', '🍧', '🍨', '🍦', '🥧', '🧁', '🍰', '🎂', '🍮', '🍭', '🍬', '🍫', '🍿', '🍩', '🍪', '🌰', '🥜', '🍯', '🥛', '🍼', '☕', '🫖', '🍵', '🧃', '🥤', '🧋', '🍶', '🍺', '🍻', '🥂', '🍷', '🥃', '🍸', '🍹', '🧉', '🍾', '🧊', '🥄', '🍴', '🍽️', '🥣', '🥡', '🥢', '🧂'],
...
function handleAction(action, emoji = null) {
  if (action === 'emoji' && !emoji) {
    ...
    // Insert emoji when clicking an emoji from the picker
